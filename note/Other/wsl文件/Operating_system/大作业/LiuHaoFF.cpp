...
int retrench()
{
    ...
    while (!(find_full == memoryTable.end() || find_null == memoryTable.end()))
    {
        tempStart = -1;
        ...
        if (find_full->state == 1 && find_null->state == 0)
        {
            ...
            for (auto &it : jobTable)
            {
                if (it.start == tempStart)
                {
                    it.start = find_null->start;
                }
            }
